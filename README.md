...
```mermaid
flowchart TB
    App["애플리케이션 코드 (3rd party 백업 프로그램)"]
    API["libcubridbackupapi.so<br/>(cubrid_backup.conf 읽음)"]
    BK["cubrid backupdb<br/>(CUBRID 유틸리티 · 요청만 전달)"]
...
복구는 방향이 반대입니다. 애플리케이션이 보관하던 백업 데이터를 API에 전달하면, API가 이를 **백업 볼륨 파일**로 재구성합니다. 그 파일을 이용한 실제 데이터베이스 복구는 CUBRID의 `cubrid restoredb` 명령으로 수행해야 합니다.
...
여기서 **drain 스레드**는 named pipe를 상시 읽어내기 위해 API가 내부적으로 만드는 전용 스레드입니다. 이 스레드는 백업을 병렬로 수행하지 않으며, 오직 파이프를 비워 서버가 쓰기에서 막히지 않게 하는 역할만 합니다. 작동방식은 [2.3](#23-계층형-버퍼-작동-방식)에서 설명합니다.
...
> **주의** 정의되지 않은 키, 섹션 헤더보다 앞에 온 항목, 또는 **타입이 맞지 않는 값**(예: 숫자 항목에 문자)은 `cubrid_backup_initialize()`가 실패합니다. 다만 **허용되지 않는 문자가 포함된 값이나 `key = value` 형태로 파싱되지 않는 줄은 오류가 아니라 조용히 무시되어, 그 설정은 기본값으로 남습니다.**
> 
...
**I/O 단위**란 API가 한 번에 읽고 쓰는 기준 크기로, 임시 작업 디렉터리가 속한 파일 시스템의 블록 크기 × 8입니다. 일반적인 Linux 파일 시스템에서는 4KB × 8 = **32KB**입니다.
...
| 정의되지 않은 키, 섹션 헤더보다 앞의 항목, 타입이 맞지 않는 값 | `cubrid_backup_initialize()` 실패 |
| 허용되지 않는 문자가 든 값 · `key=value`로 파싱되지 않는 줄 | 조용히 무시(기본값 유지) |
...
- 반환값이 `0`(완료)이면 `data_len`은 **항상 `0`**입니다. 데이터와 완료 신호가 같은 호출에 함께 오지 않으므로, `0`을 받으면 바로 반복을 종료하면 됩니다.
...
| `lookahead` | 파서가 경계를 찾기 위해 스트림을 미리 살펴본(look-ahead) 횟수. |
